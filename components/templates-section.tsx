      ...
      <div className="container px-4 md:px-6">
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 py-12 md:grid-cols-2 lg:grid-cols-3">
          {templates.map((template) => (
            <div
              key={template.id}
              className="group relative flex flex-col items-center space-y-4 rounded-lg border bg-white p-6 shadow-sm transition-all hover:shadow-md"
            >
              {/* Template Preview */}
              <div className="w-full h-64 rounded-lg shadow-md overflow-hidden">{template.preview}</div>

              <h3 className="text-xl font-bold">{template.name}</h3>
              <p className="text-center text-gray-500">{template.description}</p>
